...
while True:
    #1. Import image
    success, img = cap.read() # get the frame here
    img = cv2.flip(img, 1) # for some reason flip it ????

    #2. Find hand landmarks
    img = detector.findHands(img) # get the position of things to draw
    ...
    if len(lmList)!=0:
        ...
        x1, y1 = lmList[8][1:] # these are the pos of the brush
        x2, y2 = lmList[12][1:]



        #3. Check which finger is up
        fingers = detector.fingerUp() # weather to draw or not ???


        #4. If selection mode - two fingers are up
        if fingers[1] and fingers[2]: # if we are selecting the mode and not drawing then
            xp, yp = 0, 0
